...
export const alumniUpdateSchema = yup.object({
    firstName: yup.string().trim(),
    lastName: yup.string().trim(),
    password: yup.string().trim().min(6, "Password must be at least 6 characters"),
    designation: yup.string().trim(),
    salaryPackage: yup.string().trim(),
    file: yup.object({
        data: yup.mixed().required("File data is required").optional(),
        contentType: yup
            .string()
            .trim()
            .required("Content type is required")
            .matches(FILEREGEXP, "Only png, jpg, jpeg, or pdf files are allowed").optional(),
    }).nullable().optional(),
    rounds: yup.array()
        .of(
            yup.object({
        ...
        .min(1, "At least one round experience is required")
        .optional(),
    generalDesc: yup.string().trim(),
    socialMediaProfiles: yup.array()
        .of(
            yup.object({
                name: yup.string().trim(),
                link: yup.string().trim().url("Must be a valid URL"),
            })
        )
        .optional(),
});
